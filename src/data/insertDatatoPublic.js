...
const existingMatchIds = loadExistingData(MATCH_IDS_FILE).map((id) => id.matchId);
...
for (let d = new Date(START_DATE); d <= END_DATE; d.setDate(d.getDate() + 1)) {
  ...
  const endHour = 24; // ✅ 24시 포함

  // 코트별 매치 생성
  Object.keys(courtData).forEach((courtId) => {
    for (let hour = startHour; hour <= endHour; hour++) { // ✅ 24시 포함
      let matchDate = new Date(`${dateStr}T${String(hour).padStart(2, '0')}:00:00.000Z`);

      // ✅ 24시는 날짜를 하루 증가시키고 00:00으로 변환해야 함
      if (hour === 24) {
        matchDate.setDate(matchDate.getDate() + 1); // 날짜 하루 증가
        matchDate.setHours(0); // 00시로 변경
      }
